...
public class MapRExample3 { 
	 ...
	 public static class MyMapper extends TableMapper<Text, IntWritable> {
		 ...
		 public void map(ImmutableBytesWritable row, Result value, Context context) throws IOException, InterruptedException {
				...
				String isbn = new String(value.getValue(Bytes.toBytes("ratings"), Bytes.toBytes("isbn"))); 
				if(isbn.equals("ISBN"))
					return;
				String userid = new String(value.getValue(Bytes.toBytes("ratings"), Bytes.toBytes("userid")));
				String rating = new String(value.getValue(Bytes.toBytes("ratings"), Bytes.toBytes("rating")));
			 text.set(userid); 
		 ...
		 public void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException { 
			 int sum=0;
				...
				System.out.println("UserId:"+key+"\t"+"Number of Votes: "+count);
				Put put = new Put(Bytes.toBytes(key.toString()));
				put.add("details".getBytes(), "active".getBytes(), Integer.toString(count).getBytes());
			 
		 context.write(null, put); } }
	
	
	
	public void mapJob3() throws IOException, ClassNotFoundException, InterruptedException{
		Configuration config = HBaseConfiguration.create(); 
		Job job = new Job(config, "ExampleSummary"); 
		job.setJarByClass(MapRExample3.class);
		// class that contains // mapper and reducer 
		String sourceTable = "ratingsdb"; 
		String targetTable = "outputtemp3"; 
		Scan scan = new Scan(); 
		scan.setCaching(500); 
